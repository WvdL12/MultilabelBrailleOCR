...
def validate_int(int_label):
    ...
    assert 0 <= int_label < 64, "Ошибочная метка: " + str(int_label)
...
def unicode_to_int(unicode_lbl):
    ...
    assert 0 <= int_lbl < 64, f"incorrect unicode braille char: '{unicode_lbl}'"
    return int_lbl
...
def int_to_ascii(int_lbl):
    assert 0 <= int_lbl < 64, f"incorrect int braille char: '{int_lbl}'"
...
def human_label_to_int(label):
    ...
    if label[0] == '~':
        ...
        ch_list = reverse_dict.get(label, None)
        if not ch_list:
