...
from model_utils import class_balance_resample
...
log_path_ml = os.path.join(ML_LOGS, f"cb_{datetime.now().strftime(format='%Y%m%d_%H%M%S')}_elite_set.json")
log_path_mc = os.path.join(MC_LOGS, f"cb_{datetime.now().strftime(format='%Y%m%d_%H%M%S')}_elite_set.json")
...
for f, (train_ix, val_ix) in enumerate(kfold_split):
    trainY = train_Y[train_ix].copy()
    undersample_candidates_fold = undersample_candidates[train_ix]
    augments[f] = class_balance_resample(trainY, undersample_candidates_fold, EPSIL)
