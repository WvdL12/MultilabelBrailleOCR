...
from model_utils import label_balance_resample
...
log_path_ml = os.path.join(ML_LOGS, f"lb_{datetime.now().strftime(format='%Y%m%d_%H%M%S')}_elite_set.json")
log_path_mc = os.path.join(MC_LOGS, f"lb_{datetime.now().strftime(format='%Y%m%d_%H%M%S')}_elite_set.json")
...
for f, (train_ix, val_ix) in enumerate(kfold_split):
    ...
    augments[f] = label_balance_resample(trainY, undersample_candidates_fold, EPSIL)
